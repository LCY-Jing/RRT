...
import time
import os
from RRTPCN.Gdrawer import draw_results
from NewRRT.InverseSequence import InverseSquence
from RRTPCN.GHalton import GHalton
...
class RRTPlanner():
    ...
    def initialise(self, environment, bounds, start_pose, goal_region, object_radius, steer_distance, num_iterations, resolution, runForFullIterations,rand_fre,num_randpoint,angel,data,node_k,pcn):
        ...
        self.V = []                                         # 建立新的无元素重合的集合
        self.VPCN=[]                                          # 加入节点的状态信息的节点集合定义成列表因为元组不可修改
    ...
    def RRT(self, environment, bounds, start_pose, goal_region, object_radius, steer_distance, num_iterations,
            resolution, drawResults, runForFullIterations,rand_fre,num_randpoint,angel,data,node_k,pcn, RRT_Flavour= "RRT"):
        ...
        # 初始化参数
        self.initialise(environment, bounds, start_pose, goal_region, object_radius, steer_distance, num_iterations,
                        resolution, runForFullIterations,rand_fre,num_randpoint,angel,data,node_k,pcn)
    ...
    def RRTSearch(self):
        ...
        tree_size = 0
        path_size = 0
        self.V.append(self.start_pose)                             # 将起点加入随机树（节点集合中）
        self.VPCN.append((self.pcn_i,self.start_pose,0,0,0))               # 将索引值，起点，起点状态值，子节点数,碰撞次数加入节点集合中
        ...
        for i in range(self.N):       # N是迭代次数
            ...
            # 通过建立与目标向量之间的距离值选取前四个
                thea=[]
                thea.clear()
                for randompoint_i in random_point1:                            # 遍历随机点
                    nearest_pointtem = self.find_nearest_point(randompoint_i)
                    nearest_point = nearest_pointtem[1]
                    dis = self.euclidian_dist(randompoint_i,self.goal_pose)     # 计算采样池中采样点与目标点的距离
                    thea.append((dis,nearest_point,randompoint_i,nearest_pointtem))             # 将计算出的距离值加入列表中
                thea.sort()                                                     # 将thea元素按照dis升序排列
                for thea_tem in thea:
                    ...
                    if self.nodefileter(new_point, nearest_point, self.V):
                        ...
                        # 更新父节点状态和节点的子节点数
                            ...
                            # else:
                            self.VPCN[tem[0]] = (tem[0], nearest_point, self.VPCN[tem[0]][2] + 1/(self.VPCN[tem[0]][3]+1),
                                                 self.VPCN[tem[0]][3],self.VPCN[tem[0]][4])
        ...
        print(num_i)
        uniPruningPath=uniPruningPathAngel1=path
        return [path,uniPruningPath,uniPruningPathAngel1,num_i,self.VPCN]
    ...
    ******************************************************************************************************************************************
    """
    # 节点过滤
    def nodefileter(self,xnew,xparent,Tree):
        """
        Args:
            xnew: 当前新节点
            xparent: 当前节点的父节点
        ...
        Returns:
        """
        length =  self.euclidian_dist(xnew,xparent)
        for point in self.V:
            if point==xparent:
                continue
            if self.euclidian_dist(xnew,point)<self.node_k*length:       # 该节点被过滤掉
                return False
        return True
