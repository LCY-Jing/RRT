...
def plot_environment(env, bounds=None, figsize=None):       # bound 是画出的图形的大小
    if bounds is None and env.bounds:
        minx, miny,minz, maxx, maxy,maxz = env.bounds
    elif bounds:
    ...
    # for i, obs in enumerate(env.obstacles):            # 目前障碍物显示这有一些错误先注释掉
...
# 画随机树 gray是灰色
def plot_line(ax, edge):
    x = [p[0] for p in edge]
    y = [p[1] for p in edge]
    z = [p[2] for p in edge]


    ax.plot(x, y,z, color='gray', linewidth=1, solid_capstyle='butt', zorder=1)

def plot_poly(ax, poly, color, alpha=1.0, zorder=1):
    patch = PolygonPatch(poly, fc=color, ec="black", alpha=alpha, zorder=zorder)     # poly是三维的这里会报错
    ax.add_patch(patch)

class Environment:
    def __init__(self, yaml_file=None, bounds=None):         # 导入yaml_file的数据
