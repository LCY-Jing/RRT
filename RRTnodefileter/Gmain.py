...
from RRTnodefileter.Genvironment import Environment        # 从environment文件中导入Environment
from RRTnodefileter.GRRTs import RRTPlanner                # 从文件RRTs文件中导入RRTPlanner
from matplotlib import pyplot as plt
import math


environment = Environment('bugtrap6.yaml')
bounds = (0, 0, 10,10)
start_pose = (0.2, 0.2)
...
path= sbpp.RRT(environment, bounds, start_pose, goal_region, object_radius, steer_distance, num_iterations,
               resolution, drawResults, runForFullIterations,rand_fre,num_randpoint,angel_min,data,node_k) # 产生路径
plt.show()                   # 显示路径图
